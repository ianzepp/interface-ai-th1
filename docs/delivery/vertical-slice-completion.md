...
Run units in this order: `VS-01` → (`VS-02` and `VS-04` in parallel) → `VS-03` → `VS-05` → `VS-06` → `VS-07` → `VS-08` → `VS-09` → `VS-10` → `VS-11` → `VS-12`. `VS-04` must follow `VS-01`: both own `src/authoring/run-recorder.ts` and `tests/run-recorder.test.ts`. `VS-03` follows both `VS-02` and `VS-04`, because it also owns those recorder seams. `VS-10` follows all implementation and replay work. `VS-11` follows `VS-08` and `VS-10`. `VS-12` follows `VS-11`; this preserves the existing VS-01 through VS-11 order and scopes while making the newly admitted recovery proof the final completion work. No other unit writes `src/runtime/engine.ts`: VS-01 and VS-09 expressly exclude runtime source, and VS-02 through VS-11 do not name it. Do not put concurrent writers on `src/authoring/interactive-playwright-session.ts`, `src/authoring/run-recorder.ts`, `tests/run-recorder.test.ts`, or `src/runtime/engine.ts`.

A Hand can start immediately on `VS-01`. Once `VS-01` is complete, Hands may start `VS-02` and `VS-04` concurrently because their write scopes are disjoint. `VS-12` has no concurrent unit because it adds the final engine/result/schema contract and its target-facing proof.
...
### VS-12 — Declare, bound, report, and prove a LedgerSMB interstitial recovery

- **one change:** Add an explicit, bounded recovery-transition contract to deterministic replay and use it in a reviewed LedgerSMB inventory-lifecycle artifact for the observed disposable-password expiry interstitial.
- **write_scope:** `src/runtime/state-machine.ts`, `src/runtime/engine.ts`, `src/runtime/result.ts`, `schemas/capability.schema.json`, `schemas/result.schema.json`, `src/capabilities/ledgersmb-inventory-lifecycle.ts`, `src/runtime/ledgersmb-inventory-lifecycle-replay.ts`, `src/authoring/ledgersmb-lifecycle-pilot.ts`, `package.json`, `tests/engine.test.ts`, `tests/schemas.test.ts`, `tests/artifact-quality.test.ts`, `runs/`, `evidence/runs/`.
- **edit:** Add an optional `recovery` declaration to `StageTransition`, with a stable recovery id, named observed condition, source run id, bounded `maxAttempts`, and the existing explicit destination; the engine counts each declaration per run, records an observer recovery event plus a `recoveries` array on every typed `RunResult`, and returns typed `failure` code `recovery-exhausted` before another recovery action when that declaration reaches its cap. Extend the capability schema for the optional transition declaration and the result schema for the optional recovery report, preserving all existing artifacts and promoted legacy result files; author `ledgersmb.inventory-lifecycle` so its post-login detector routes the observed `OK` password-expiry interstitial through one reversible dismiss stage and then to the normal first-menu stage, and add its deterministic replay command.
