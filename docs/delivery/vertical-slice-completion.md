...
Run units in this order: `VS-01` → `VS-02` → `VS-03` → `VS-05` → `VS-06` → `VS-07` → `VS-08` → `VS-09`.  `VS-04` may run concurrently with `VS-01` because its write scope is disjoint; it must finish before `VS-05`.  `VS-10` follows all implementation and replay work.  `VS-11` is last.  Do not put concurrent writers on `src/authoring/interactive-playwright-session.ts`.

A Hand can start immediately on `VS-01` and, in parallel, `VS-04`.

## Settled design decisions

1. **Provenance recording — choose harness-attested producer and decision receipts.**  Add a structured producer record to `TestRunManifest` with immutable session identity, producer kind (`external-llm` or `human`), provider, model, model-session/request identity, and a harness-generated session nonce.  Add a decision-receipt field to `proposal` and an explicit rejected-decision event to `DiscoveryEvent`; each receipt identifies the producer, binds the prior observation event hash/sequence, the proposed command hash, and the harness-generated nonce.  `discovery-session-cli.ts` obtains model identity and an invocation/response receipt from the configured external-model host at session creation, creates the nonce, and passes the resulting sealed identity into `SessionOptions`; neither JSONL stdin nor the socket protocol accepts producer, model, receipt, nonce, or sequence fields.  The recorder computes the append-only receipt chain and writes the terminal chain digest to the manifest.  A review tool validates the chain, that every action has an earlier model decision bound to the observed state, and that the discovery run has a non-human model receipt before promotion.  This is checkable evidence of a specific model-host invocation, not a controller-authored assertion.  Reject a free-text `model` field sent by the controller because it is forgeable and cannot bind a particular decision to the observation that preceded it.
...
- **write_scope:** `src/authoring/interactive-playwright-session.ts`, `src/authoring/event-recorder.ts`, `src/authoring/run-recorder.ts`, `src/authoring/discovery-session-cli.ts`, `tests/interactive-playwright-session.test.ts`, `tests/run-recorder.test.ts`.
- **edit:** Add sealed producer/receipt and rejected-decision shapes to the existing session, ledger, and manifest seams; have the launcher—not either command transport—supply model-host identity and the recorder persist/check the chain digest.
- **done_when:** A finalized external-LLM run names an attested model-host receipt and chain digest; each accepted action has a prior bound observation and proposal receipt; controller-supplied producer metadata is rejected and cannot alter `run.json`.
- **validation:** `npm test -- --test-name-pattern="interactive|run"`.
- **depends_on:** none.
...
- **validation:** `npm test -- --test-name-pattern="interactive|session-control"`.
- **depends_on:** VS-01.
...
- **validation:** `npm test -- --test-name-pattern="redaction|run-recorder|playwright"`.
...
- **validation:** `npm test -- --test-name-pattern="evidence-promotion|run-recorder"`.
- **depends_on:** VS-01.
...
- **validation:** `npm test -- --test-name-pattern="interactive|session-state|control-lease"`.
...
- **validation:** `npm test -- --test-name-pattern="interactive|session-control|session-state"`.
- **depends_on:** VS-05.
...
- **validation:** `npm test -- --test-name-pattern="interactive|control-lease|state-machine"`.
...
- **validation:** `npm test -- --test-name-pattern="policy|redaction|run-recorder|playwright"`.
- **depends_on:** VS-03.
...
- **write_scope:** `tests/schemas.test.ts`, `tests/artifact-quality.test.ts`, `src/audit/artifact-rubric.ts`, `schemas/capability.schema.json`, `schemas/invocation.schema.json`, `schemas/result.schema.json`, `src/runtime/result.ts`, `src/runtime/engine.ts`, `tests/engine.test.ts`, `tests/state-machine.test.ts`.
- **edit:** Add only compatibility/regression assertions needed to prove existing `intervention-required` results and all three artifacts remain valid; leave schema files and runtime source byte-for-byte unchanged unless a test exposes a real contract mismatch.
- **done_when:** The three committed artifacts validate against the published capability schema, invocation/result fixtures validate, and an engine intervention terminal outcome remains typed and schema-valid.
- **validation:** `npm test -- --test-name-pattern="schemas|artifact-quality|engine|state-machine"`.
- **depends_on:** VS-07.
- **do_not:** Do not version-bump schemas for per-run ledger data, weaken artifact validation, or edit capability artifacts merely to make an unrelated test pass.
...
- **validation:** `git grep -n -i -E "password|secret|authorization" -- evidence/runs && npm run verify`.
