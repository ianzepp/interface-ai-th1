...
export function buildAuthorPrompt(options: AuthorPromptOptions): string {
    return [
        ...
        "- **Reserve at least two of them for validation** — one happy-path replay and",
        "  one per admitted branch. Those runs are not available for experiments. A",
        "  capability with no successful replay is not validated, however much",
        "  exploration it holds, so spend the exploration budget first and keep the",
        "  validation reserve intact.",
